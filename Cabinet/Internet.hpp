...
public:
	...
	DWORD ConnectServer(BOOL* pb_Offline)
	{
		*pb_Offline = FALSE;
		...
		#if _TraceInternet
			TraceW(L"Connecting: %s:%u", mu16_Server, mu16_Port);
		#endif
	...
	DWORD HttpOpenFile(WCHAR* u16_Method, void* p_POST, DWORD u32_POSTLength, WCHAR* u16_GET, DWORD* pu32_Status)
	{
		*pu32_Status = 0;
		...
		#if _TraceInternet
			TraceW(L"HTTP Open: %s%s", mu16_Server, (WCHAR*)s_Url);
		#endif
	...
	DWORD FtpOpenFile(DWORD u32_FirstByte)
	{
		...
		#if _TraceInternet
			TraceW(L"FTP Open: %s%s", mu16_Server, mu16_UrlPath);
		#endif
	...
	DWORD ReadFile(void* p_Buffer, DWORD u32_Count, DWORD* pu32_Read)
	{
		*pu32_Read = 0;
		...
		#if _TraceInternet
			TraceW(L">>>>>>>>>>>>>>>>>>>>>>>>>> Downloaded %d Bytes with %d kB/s <<<<<<<<<<<<<<<<<<<<<<<<<", *pu32_Read, *pu32_Read/s32_Tick);
		#endif
		return 0;
	}
	...
	void DeleteTempFile()
	{
		...
		#if _TraceInternet
			if (mu16_TempFile[0]) TraceW(L"*** Deleting Temp file: %s", mu16_TempFile);
		#endif
	...
	void CloseInternet()
	{
		...
		if (mh_Connection)
		{
			...
			#if _TraceInternet
				TraceW(L"Internet Close Connection");
			#endif
		}
		if (mh_Session)    
		{
...
public:
	...
	DWORD CreateDownloadFileW(WCHAR* u16_File)
	{
		...
		#if _TraceInternet
			TraceW(L"Creating download file: %s", u16_File);
		#endif
		...
		#if _TraceInternet
			TraceW(L"*** Successfully created Download file: %s", u16_File);
		#endif
		return 0;
	}
	...
	DWORD DownloadEntireFileToDisk(DWORD *pu32_Status)
	{
		#if _TraceInternet
			TraceW(L"--------------------------------------");
			TraceW(L"Internet Start downloading entire file to disk.");
		#endif
	...
	DWORD DownloadFilePartToMemory(void* p_Buffer, DWORD u32_Offset, DWORD u32_Count, DWORD* pu32_Read, DWORD* pu32_Status)
	{
		#if _TraceInternet
			TraceW(L"--------------------------------------");
			TraceW(L"Internet Start downloading file part to memory. Offset= %u Byte, Count= %u Byte", u32_Offset, u32_Count);
		#endif
