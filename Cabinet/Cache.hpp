...
public:
	...
	void Init(int s32_BlockSize, fReadData f_ReadCallback, WCHAR c_CacheName, WCHAR c_BlockName)
	{
		...
		if (ms32_BlockSize != s32_BlockSize)
		{
			...
			mu8_Memory = 0;
			if (s32_BlockSize) mu8_Memory = new BYTE[s32_BlockSize];
		}
	}
	...
	{
		#if _TraceCache
			TraceW(L"  CMemBlock %s:  ReadData(Pos= %08d, Count= %05d)", mu16_Name, s32_Pos, s32_Count);
		#endif
		...
		{
			#if _TraceCache
				if (s32_Pos < ms32_StartPos) TraceW(L"******* Warning Cache Callback reads a block twice! *******");
			#endif
			...
			#if _TraceCache
				TraceW(L"  CMemBlock %s: *Callback(Pos= %08d, Count= %05d) --> %05d Bytes read", mu16_Name, s32_Pos, ms32_BlockSize, ms32_Content);
			#endif
		...
		#if _TraceCache
			TraceW(L"  CMemBlock %s:  Copy Buf RelPos= %05d, Count= %05d", mu16_Name, s32_RelPos, s32_Count);
		#endif
...
public:
	...
	{
		#if _TraceCache
			TraceW(L" CCache:         ReadData(Pos= %08d, Count= %05d)", s32_Pos, s32_Count);
		#endif
		...
		{
			#if _TraceCache
				TraceW(L" CCache --> %05d Bytes read", s32_Read);
			#endif
			return s32_Read;
		}
		...
		#if _TraceCache
			TraceW(L" CCache --> %05d + %05d = %05d Bytes read", s32_Read, s32_Rest, s32_Read+s32_Rest);
		#endif
