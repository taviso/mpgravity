...
public:
	...
	BOOL ExtractFileW(CStrW sw_CabPath, CStrW sw_TargetDir, void* pParam = NULL)
	{
		...
		{
			#if _TraceExtract
				TraceW(L"++++++++++++\nFDICopy ('%s') -> starting extraction", (WCHAR*)sw_CabFile);
			#endif
		...
		{
			#if _TraceExtract
				TraceW(L"*** Extraction of '%s' has been aborted. Now deleting incomplete file.", (WCHAR*)ms_CurrentFile);
			#endif
	...
	BOOL IsCabinetW(CStrW sw_CabPath, PFDICABINETINFO pfdici = NULL)
	{
		if (!mh_FDIContext)
			...
			TraceW(L"++++++++++++\nFDIIsCabinet('%s')", (WCHAR*)sw_FileName);
		#endif
...
protected:
	...
	static CExtract*& This()
	{
		static CExtract* p_This = 0;
		return p_This;
	}
...
private:
	...
	void* FdiAlloc(UINT size)
	{ 
		return Alloc(size);
	}

	void FdiFree(void* memblock)
	{ 
		Free(memblock);
	}
	...
	INT_PTR FdiOpenW(WCHAR* u16_File, int oflag, int pmode)
	{
		...
		else
		{
			...
			TraceW(L"FDIOpen (%s, '%s') --> Handle= 0x%08X", (b_Write ? L"for WRITE" : L"for READ"), (WCHAR*)sw_FileName, fd);
		#endif
		return fd;
	}
	...
	{ 
		#if _TraceExtract
			TraceW(L"FDIRead (Handle= 0x%08X, Count= %05d)", fd, count);
		#endif
		...
		if (mi_Blowfish.IsPasswordSet()) // encrypted
		{
			...
			BYTE u8_Buffer[64*1024]; // big enough because Cabinet.dll uses a 32 kB buffer
			s32_Read = Read(fd, u8_Buffer, s32_Count);
			if (s32_Read < s32_Count)
			{
				#if _TraceExtract
					TraceW(L"FDIRead (Handle= 0x%08X) ERROR: %d instead of %d Bytes read. (position 0x%X - 0x%X)", fd, s32_Read, s32_Count, s32_CabPtr, s32_CabPtr+s32_Count);
				#endif
				return -1;
			}
			...
			mi_Blowfish.CryptBlocks(FALSE, u8_Buffer+s32_UnEncrypted, (s32_Count-s32_UnEncrypted)/8);

			memcpy(memory, u8_Buffer + s32_Offset, count);

			// move filepointer to where Cabinet.dll expects it to be
			Seek(fd, s32_CabPtr + count, SEEK_SET);

			#if _TraceExtract
				TraceW(L"FDIRead --> %05d Bytes read, %05d Bytes decrypted, %05d Bytes copied to buffer at position 0x%05X", s32_Read, s32_Count-s32_UnEncrypted, count, s32_CabPtr);
			#endif
		...
		else // not encrypted
		{
			...
			#if _TraceExtract
				TraceW(L"FDIRead --> %05d Bytes read", s32_Read);
			#endif
		}
	...
	{ 
		#if _TraceExtract
			TraceW(L"FDIWrite(Handle= 0x%08X, Count= %05d)", fd, count);
		#endif
		...
		#if _TraceExtract
			TraceW(L"FDIWrite --> %05d Bytes written", s32_Written);
		#endif
	...
	{ 
		...
		#if _TraceExtract
			TraceW(L"FDIClose(Handle= 0x%08X)", fd);
		#endif
	...
	{ 
		int Pos = Seek(fd, offset, seektype); 
			...
			switch (seektype)
			{
				case SEEK_SET: TraceW(L"FDISeek (Handle= 0x%08X, SEEK_SET, Offset= %08d) --> Position= %08d", fd, offset, Pos); break;
				case SEEK_CUR: TraceW(L"FDISeek (Handle= 0x%08X, SEEK_CUR, Offset= %08d) --> Position= %08d", fd, offset, Pos); break;
				case SEEK_END: TraceW(L"FDISeek (Handle= 0x%08X, SEEK_END, Offset= %08d) --> Position= %08d", fd, offset, Pos); break;
			}
		#endif
	...
	INT_PTR FdiCallback(FDINOTIFICATIONTYPE fdint, PFDINOTIFICATION pfdin)
	{
		...
		switch (fdint)
		{
			...
			{
				#if _TraceExtract
					TraceW(L"FDICallback(CABINET_INFO)");
				#endif
			...
			{
				#if _TraceExtract
					TraceW(L"FDICallback(NEXT_CABINET)");
				#endif
			...
			case fdintCOPY_FILE:
			{
				...
				#if _TraceExtract
					TraceW(L"FDICallback(COPY_FILE, '%s', Size= %d)", (WCHAR*)sw_File, pfdin->cb);
				#endif
				...
				if (mk_Callbacks.f_OnBeforeCopyFile)
				{
					...
					{
						#if _TraceExtract
							TraceW(L"*** OnBeforeCopyFile() callback has canceled extraction.");
						#endif
						break;
					}
				}
				...
				if (mb_ExtractToMemory)
				{
					...
					#if _TraceExtract
						TraceW(L"Extracting to MEMORY --> Dummy Handle= 0x%08X", u32_DummyHandle);
					#endif
					nRet = u32_DummyHandle;
				}
				else // Extract to file
				{
					CFile::CreateFolderTreeW(sw_Path);
			...
			case fdintCLOSE_FILE_INFO:
			{
				...
				sw_Path += sw_RelPath;
					...
					TraceW(L"FDICallback(CLOSE_FILE_INFO, '%s', Handle= 0x%X)", (WCHAR*)sw_FileName, pfdin->hf);
				#endif
			...
			#if _TraceExtract
				case fdintPARTIAL_FILE: TraceW(L"FDICallback(PARTIAL_FILE)");   break;
				case fdintENUMERATE:    TraceW(L"FDICallback(ENUMERATE)");      break;
				default:                TraceW(L"FDICallback(unknown action)"); break;
			#endif
		}

		#if _TraceExtract
			TraceW(L"FdiCallback() --> 0x%08X\n-----------------------", nRet);
		#endif
		return nRet;
	}
...
protected:
	...
	void SetAttribsAndDateW(WCHAR* u16_File, USHORT uDate, USHORT uTime, USHORT uAttribs)
	{
		...
		BOOL b_Utc = (uAttribs & FILE_ATTR_UTC_TIME) > 0;
			...
			CStrW sw_FileName;
			CFile::SplitPathW(u16_File, 0, &sw_FileName);
			TraceW(L"SetAttribsAndDateW('%s'), UTC=%d, UTF=%d", (WCHAR*)sw_FileName, b_Utc, b_Utf);
		#endif
		...
		{
			#if _TraceExtract
				TraceW(L"SetAttribsAndDateW('%s') ***** Error %d opening file!", u16_File, GetLastError());
			#endif
			return;
		}
