...
public:
	...
	BOOL CreateFCIContextW(CStrW   sw_CabFile,                   // "C:\Temp\Packed.cab"
	                       ...
	                       void*   pParam=NULL)                  // optional user parameter
	{
		CStrW sw_Folder, sw_File;
		CFile::SplitPathW(sw_CabFile, &sw_Folder, &sw_File);

		#if _TraceCompress
			TraceW(L"*** FCICreate(File='%s', UTC=%d, Split=0x%X, ID=%d) ***", (WCHAR*)sw_File, b_UtcTime, u32_CabSplitSize, u16_CabID);
		#endif
	...
	BOOL DestroyFCIContext(void* pParam = NULL)
	{
		BOOL b_Ret = TRUE;
	...
	BOOL AddFileW(CStrW sw_FileToAdd, CStrW sw_NameInCab, BOOL b_Compress=TRUE, void* pParam=NULL)
	{
		if (!mh_FCIContext)
			return FALSE;
		...
		#if _TraceCompress
			TraceW(L"*** FCIAddFile('%s') ***", u16_FileToAdd);
		#endif
	...
	BOOL AddFolderW(CStrW sw_Path, CStrW sw_Filter=L"*.*", BOOL b_Compress=TRUE, void* pParam=NULL, int s32_BaseLen=-1)
	{
		...
		do
		{
			...
			{
				if (!AddFolderW(sw_File, sw_Filter, b_Compress, pParam, s32_BaseLen))
				{
					FindClose(h_File);
					return FALSE;
				}
			...
			{
				if (!AddFileW(sw_File, (WCHAR*)sw_File + s32_BaseLen, b_Compress, pParam))
				{
					FindClose(h_File);
					return FALSE;
				}
...
protected:
	...
	// declaration of static class pointer (avoids Linker errors)
	static CCompress*& This()
	{
		static CCompress* p_This = 0;
		return p_This;
	}
...
private:
	...
	static void* FCIAlloc(UINT size)
		{ return This()->FciAlloc(size); }

	static void FCIFree(void* memblock)
		{ This()->FciFree(memblock); }
	
	static INT_PTR FCIOpen(char* s8_File, int oflag, int pmode, int *err, void *pv)
		{ return This()->FciOpenA(s8_File, oflag, pmode, err, pv); }
	
	static UINT FCIRead(INT_PTR fd, void *memory, UINT count, int *err, void *pv)
		{ return This()->FciRead(fd, memory, count, err, pv); }
	
	static UINT FCIWrite(INT_PTR fd, void *memory, UINT count, int *err, void *pv)
		{ return This()->FciWrite(fd, memory, count, err, pv); }
	
	static int FCIClose(INT_PTR fd, int *err, void *pv)
		{ return This()->FciClose(fd, err, pv); }
	
	static long FCISeek(INT_PTR fd, long offset, int seektype, int *err, void *pv)
		{ return This()->FciSeek(fd, offset, seektype, err, pv); }
	
	static int FCIDelete(char *s8_File, int *err, void *pv)
		{ return This()->FciDelete(s8_File, err, pv); }
	
	static BOOL FCIGetTempFile(char *pszTempName, int cbTempName, void *pv)
		{ return This()->FciGetTempFile(pszTempName, cbTempName, pv); }
	
	static INT_PTR FCIGetAttribsAndDate(char *pszName, USHORT *pdate, USHORT *ptime, USHORT *pattribs, int *err, void *pv)
		{ return This()->FciGetAttribsAndDate(pszName, pdate, ptime, pattribs, err, pv); }
	
	static int FCIFilePlaced(PCCAB pccab, char *s8_File, int s32_FileSize, BOOL fContinuation, void *pv)
		{ return This()->FciFilePlaced(pccab, s8_File, s32_FileSize, fContinuation, pv); }
	
	static BOOL FCIGetNextCabinet(PCCAB pccab, ULONG cbPrevCab, void* pv)
		{ return This()->FciGetNextCabinet(pccab, cbPrevCab, pv); }
	
	static long FCIUpdateStatus(UINT typeStatus, ULONG cb1, ULONG cb2, void *pv)
		{ return This()->FciUpdateStatus(typeStatus, cb1, cb2, pv); }

	// #################### MEMBER FCI CALLBACKS ########################

	void* FciAlloc(UINT size)
	{ 
		return Alloc(size); 
	}

	void FciFree(void* memblock)
	{ 
		Free(memblock); 
	}

	INT_PTR FciOpenA(char* s8_File, int oflag, int pmode, int *err, void *pv)
	{
		CStrW sw_File;
		return FciOpenW(sw_File.DecodeUtf8(s8_File), oflag, pmode, err, pv);
	}
	INT_PTR FciOpenW(CStrW sw_File, int oflag, int pmode, int *err, void *pv)
	{ 
		INT_PTR fd = Open(sw_File, oflag, pmode, err, mp_Param);
		...
		#if _TraceCompress
			TraceW(L"> > > > FCIOpen (%s, %s) --> Handle= 0x%08X", (b_Write ? L"Write" : L"Read"), (WCHAR*)sw_File, fd);
		#endif
		...
		if (mk_Encrypt.i_Blowfish.IsPasswordSet() && wcsstr(mk_Encrypt.sw_CabFiles, sw_File) > 0) 
		{
			...
			#if _TraceCompress
				TraceW(L"**** Blowfish EncryptStreamOpen");
			#endif
		}
		return fd;
	}

	UINT FciRead(INT_PTR fd, void *memory, UINT count, int *err, void *pv)
	{ 
		int s32_Read = Read(fd, memory, count, err, mp_Param); 

		#if _TraceCompress
			TraceW(L"FCIRead (Handle= 0x%08X, Count= %5d) --> %05d Bytes read", fd, count, s32_Read);
		#endif
	...
	UINT FciWrite(INT_PTR fd, void *memory, UINT count, int *err, void *pv)
	{ 
		// Write without encryption
		if (mk_Encrypt.h_CabHandle != fd)
		{
			// Write data block to disk
			UINT u32_Written = Write(fd, memory, count, err, mp_Param); 

			#if _TraceCompress
				TraceW(L"FCIWrite(Handle= 0x%08X, Count= %5d) --> %05d Bytes written", fd, count, u32_Written);
			#endif
		...
		if (mk_Encrypt.u32_Pointer < ENCRYPTION_START)
		{
			...
			UINT u32_Written = Write(fd, u8_Buffer, u32_UnEncrypted, err, mp_Param);
			if  (u32_Written != u32_UnEncrypted)
				 return -1;

			#if _TraceCompress
				TraceW(L"FCIWrite(Handle= 0x%08X) --> %05d unencrypted Bytes written", fd, u32_Written);
			#endif
		...
		if (u32_Count > 0)
		{
			BYTE   u8_Crypt[64*1024]; // big enough because Cabinet.dll uses a 32 kB buffer
			DWORD u32_Encrypted = 0;
			mk_Encrypt.i_Blowfish.EncryptStreamData(u8_Buffer, u32_Count, u8_Crypt, sizeof(u8_Crypt), &u32_Encrypted);

			UINT u32_Written = Write(fd, u8_Crypt, u32_Encrypted, err, mp_Param);
			if  (u32_Written != u32_Encrypted)
				 return -1;

			#if _TraceCompress
				TraceW(L"FCIWrite(Handle= 0x%08X) --> %05d Bytes encrypted, %05d Bytes written", fd, u32_Count, u32_Written);
			#endif
	...
	BOOL FlushBlowfish(INT_PTR fd, int *err)
	{
		...
		{
			#if _TraceCompress
				TraceW(L"**** Blowfish EncryptStreamClose (Handle= 0x%08X) --> flushed last block (8 Bytes)", fd);
			#endif

			if (u32_Flushed != Write(fd, u8_Flush, u32_Flushed, err, mp_Param))
				return FALSE;
		}
		return TRUE;
	}

	long FciSeek(INT_PTR fd, long offset, int seektype, int *err, void *pv)
	{ 
		...
		int Pos = Seek(fd, offset, seektype, err, mp_Param); 

		#if _TraceCompress
			switch (seektype)
			{
				case SEEK_SET: TraceW(L"FCISeek (Handle= 0x%08X, SEEK_SET, Offset= %05d) --> Position= %05d", fd, offset, Pos); break;
				case SEEK_CUR: TraceW(L"FCISeek (Handle= 0x%08X, SEEK_CUR, Offset= %05d) --> Position= %05d", fd, offset, Pos); break;
				case SEEK_END: TraceW(L"FCISeek (Handle= 0x%08X, SEEK_END, Offset= %05d) --> Position= %05d", fd, offset, Pos); break;
			}
		#endif
	...
	int FciClose(INT_PTR fd, int *err, void *pv)
	{ 
		#if _TraceCompress
			TraceW(L"FCIClose(Handle= 0x%08X)", fd);
		#endif
		...
		return Close(fd, err, mp_Param); 
	}


	int FciDelete(char* s8_File, int* err, void* pv)
	{ 
		CStrW sw_File;
		sw_File.DecodeUtf8(s8_File);
		
		#if _TraceCompress
			TraceW(L"FCIDelete('%s')", (WCHAR*)sw_File);
			TraceW(L"-------");
		#endif

		return Delete(sw_File, err, mp_Param); 
	}

	// see comment for GetTempFileW!
	BOOL FciGetTempFile(char *pszTempName, int cbTempName, void *pv)
	{ 
		return GetTempFileA(pszTempName, cbTempName, mp_Param); 
	}

	// see comment for GetAttribsAndDate!
	INT_PTR FciGetAttribsAndDate(char *s8_File, USHORT *pdate, USHORT *ptime, USHORT *pattribs, int *err, void *pv)
	{
		if (mb_Abort)
			return -1;

		CStrW sw_File;
		return GetAttribsAndDateW(sw_File.DecodeUtf8(s8_File), pdate, ptime, pattribs, err, mp_Param);
	}
	
	// see comment for OnFilePlaced!
	int FciFilePlaced(PCCAB pccab, char *s8_File, int s32_FileSize, BOOL fContinuation, void *pv)
	{ 
	...
	BOOL FciGetNextCabinet(PCCAB pccab, ULONG cbPrevCab, void* pv)
	{
		if (mb_Abort)
			return FALSE;
		...
		#if _TraceCompress
			TraceW(L"FCIGetNextCabinet formatted filename: '%s'", (WCHAR*)sw_CabPath);
		#endif
	...
	int FciUpdateStatus(UINT typeStatus, ULONG cb1, ULONG cb2, void *pv)
	{
		if (mb_Abort)
			return -1;
...
protected:
	...
	// Opens a file
	virtual INT_PTR Open(WCHAR* u16_File, int oflag, int pmode, int *err, void* p_Param)
	{
	...
	// Read a file
	virtual UINT Read(INT_PTR hf, void *memory, UINT cb, int *err, void* p_Param)
	{ 
	...
	// Write a file
	virtual UINT Write(INT_PTR hf, void *memory, UINT cb, int *err, void* p_Param)
	{ 
	...
	// Close a file
	virtual int Close(INT_PTR hf, int *err, void* p_Param)
	{ 
	...
	// Delete a (temp) file
	virtual int Delete(WCHAR *u16_File, int *err, void* p_Param)
	{     
	...
	virtual long Seek(INT_PTR hf, long dist, int seektype, int *err, void* p_Param)
	{ 
	...
	BOOL GetTempFileA(char *s8_TempName, int cbBufSize, void* p_Param)
	{ 
		CStrW sw_TempFile;
		while (TRUE)
		{
			sw_TempFile.Format(L"%sCabTemp%04u", (WCHAR*)msw_TempDir, ms32_TempCounter++);
	...
	INT_PTR GetAttribsAndDateW(WCHAR* u16_File, USHORT *pdate, USHORT *ptime, USHORT *pattribs, int *err, void *p_Param)
	{
		#if _TraceCompress
			CStrW s_FileName;
			CFile::SplitPathW(u16_File, 0, &s_FileName);
			TraceW(L"GetAttribsAndDate('%s'), UTC=%d, UTF=%d", (WCHAR*)s_FileName, mb_UtcTime, mb_NameInCabIsUtf);
		#endif
		...
		*pattribs = (USHORT)(k_FileInfo.dwFileAttributes & (FILE_ATTRIBUTE_READONLY | FILE_ATTRIBUTE_HIDDEN | FILE_ATTRIBUTE_SYSTEM | FILE_ATTRIBUTE_ARCHIVE));
		...
		return FciOpenW(u16_File, _O_RDONLY | _O_BINARY, _S_IREAD, err, this);
	}
};
